...
define("utils",function(){ 
	...
	var generateRandomNumber = function(min_num,max_num,precision) {
			highlightedNumber = (Math.random() * (max_num - min_num) + min_num).toFixed(precision);
	...
	var onLayerClick = function (layerButton) {
	...
	var synchronizeLayerList = function () {
	...
	var coloring = function(data,color_start,color_end){
	...
	var draw_legend = function(rainbow,max_data){
		for (i = 1; i <=10; i++){
	...
	var myTimer = function (source,color_start,color_end) {
		var data = {};
		$.getJSON(source, function(result) {
			...
			delete data.date;
			rgb_dic = coloring(data,color_start,color_end);
...
/* 	var data = [];
	for ( i = 0; i < layer_array.length; i++){
	 var ob = generateRandomNumber(0,100,3);
	 data.push(ob);
	};
	for ( i = 0; i < layer_array.length; i++){
	 layer_array[i].scene.materials.stuff.diffuse = new Float32Array([rgb_array[i][0]/255,rgb_array[i][1]/255,rgb_array[i][2]/255,1]);
	}; 
    var data = await new Promise(function(resolve) {
		var data = {};
        $.getJSON("heat.json", function(result) {
			for (key in result){
				data[key] = result[key];
			};
			resolve(data) 
